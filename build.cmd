...
echo.
echo Examples:
echo  Build a release build:                    build.cmd
echo  Build a release build, full recompile:    build.cmd -c
echo  Build a debug build and run:              build.cmd -d -r
echo  Build in debug, run, don't print at all:  build.cmd -drqq
exit /B
...
set "TEMP_DIR=_temp\release"
IF DEFINED BUILD_DEBUG (
  set "TEMP_DIR=_temp\debug"
)
